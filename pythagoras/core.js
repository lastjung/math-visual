...
const Core = {
    ...
    setupUI() {
        ...
        if (!document.getElementById('floating-dock-container')) {
            ...
            dock.innerHTML = `
                <button class="icon-btn" id="btn-prev-dock" title="이전 단계" style="display:none; font-size:18px;">◀</button>
                <button class="icon-btn" id="btn-next-dock" title="다음 단계" style="display:none; font-size:18px;">▶</button>
                <div class="dock-divider" id="dock-div-nav" style="display:none;"></div>
    ...
    updateControls() {
        ...
        if (this.currentCase && this.currentCase.uiConfig) {
            ...
            // Sync floating dock navigation buttons for mobile
            const btnPrevDock = document.getElementById('btn-prev-dock');
            const btnNextDock = document.getElementById('btn-next-dock');
            const divNav = document.getElementById('dock-div-nav');
            
            if (btnPrevDock && btnNextDock && divNav) {
                // If it's not desktop and the case has 'prev-btn'/'next-btn' in uiConfig (handled by moveStep)
                const hasNav = controls.some(c => c.id === 'prev-btn' || c.id === 'next-btn');
                const showNav = hasNav && !isDesktop;
                
                btnPrevDock.style.display = showNav ? 'flex' : 'none';
                btnNextDock.style.display = showNav ? 'flex' : 'none';
                divNav.style.display = showNav ? 'block' : 'none';
                
                btnPrevDock.onclick = () => { if(this.currentCase && this.currentCase.moveStep) this.currentCase.moveStep(-1); };
                btnNextDock.onclick = () => { if(this.currentCase && this.currentCase.moveStep) this.currentCase.moveStep(1); };
            }
        }
    ...
    resetCase() {
        ...
        if (this.currentCase.reset) {
            this.currentCase.reset();
        } else if (this.currentCase.applyStep) {
            this.currentCase.applyStep(0, false);
        }

        // Always attempt to start auto-play on reset unless explicitly off
        if (this.currentCase.autoPlayOnReset !== false) {
            this.isRunning = true;
            if (this.currentCase.start) this.currentCase.start();
            if (typeof gsap !== 'undefined') gsap.globalTimeline.play();
            this.startAutoPlay();
        } else {
            this.isRunning = false;
            if (this.currentCase.stop) this.currentCase.stop();
    ...
    startAutoPlay() {
        this.stopAutoPlay();
        if (this.currentCaseMode === 'interactive') return;
        this.autoPlayTimer = setInterval(() => {
            if (this.currentCase && this.currentCase.moveStep) {
                this.isAutoPlaying = true;
                this.currentCase.moveStep(1);
