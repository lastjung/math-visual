...
이 문서는 `math-sound` 프로젝트에 통합된 6개 카테고리, 총 40개 수식의 제목, 동작 수식, 그리고 실시간 변수(Parameter) 규칙을 상세히 기록합니다.

---

## 🚀 6대 카테고리 통합 현황 (Total: 40 Functions)
...
*   **👹 Insane (7곡)**: 광기 어린 프랙탈 간섭과 극한의 수식 조합.
...
|---|---|---|
| **Trig Tomfoolery** | `r = sin(2θ + sin(4θv))` | `v = (loopIdx % 20) * 0.314` |
...
| **Mind Blowing Spinny** | `r = sign(cos(nθ+3v)) + sin(vθ/20)` | `v = (loopIndex % 10) * 0.5, n = 6` |
...
*   **절대값 보정**: `Deadpool Geometry` 등 복합 수식 대상 대칭형 음수 반지름 복원.
