...
export const MATH_FUNCTIONS = {
    ...
    spiralGalaxy: {
        ...
        x: (t) => {
            ...
            // Scale d to reach further out
            const d = t / (2 * Math.PI); 
            const dr = E * (1 + Math.cos(2 * Math.PI * d / Lg)) / 2;
            const theta_r = da * d;
            const angle = t * 6; // Restored spin for more detail
    ...
    eventHorizon: {
        ...
        x: (t) => {
            const p = t / (2 * Math.PI);
            // Galaxy-style mechanics
            const E = 0.45; // More intense deformation
            const dr = E * (1 + Math.cos(p * Math.PI)) / 2;
            const theta_r = 0.8 * p;
            // Shrink from 6 to 0.4
            const d = Math.max(0.4, 6.0 * (1.0 - p * 1.1));
            // Standard spin + extreme acceleration at the end (45 radians ≈ 7+ extra spins)
        ...
        formula: 'Galaxy-style Horizon: R(θ, d) with final 3-spin vacuum',
    ...
    saturnRings: {
        ...
        x: (t) => {
            const p = t / (2 * Math.PI);
            // Discrete ring layers: jumps between 3.5, 4.2, 5.0
    ...
    solarPlasma: {
        ...
        r: (theta) => {
            const base = 5;
            // Reverted to the version you liked: subtle ripples + gaps
    ...
    pulsarBeam: {
        ...
        type: 'polar',
        r: (theta) => {
            // Symmetrical jets + core radiance (0.2) to prevent 'stop' feeling
